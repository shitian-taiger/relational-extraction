...
db_file_path = Path.joinpath(Path(__file__).parent, "data/store.db")
...
def process_db(filename):
    ...
    sentence_cursor.execute('SELECT * FROM {}'.format(SENTENCE_TABLE))
...
if __name__ == "__main__":
    file_name = Path.joinpath(Path(__file__).parent, "instances.txt")
